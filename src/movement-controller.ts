...
const radius = 10; // The distance from the object.
...
export class MovementController {
  ...
  private getPlayerBreathingCameraOffset = (): number => {
    ...
    const verticalOffset = Math.sin(this._breathingProgress) * breathingAmount;
  ...
  private getPlayerMovementGroundOffset = (): number => {
    // Calculate bobbing offset based on sine wave
    const verticalOffset = Math.sin(this._movementProgress) * movementAmount;
  ...
  public start = () => {
    ...
    // We should probably use a raycaster to determine the ground position.
